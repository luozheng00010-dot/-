...
import { Alert, App, Button, Card, Drawer, Empty, Form, Image, Input, Modal, Pagination, Select, Space, Spin, Tag, Typography } from "antd";
...
export default function AssetsPage() {
    ...
    const retryLoadAssets = useAssetStore((state) => state.retryLoadAssets);
    const loading = useAssetStore((state) => state.loading);
    const loadingError = useAssetStore((state) => state.loadingError);
    const scope = useOwnerScopeStore((state) => state.scope);
    const members = useOwnerScopeStore((state) => state.members);
    const membersLoading = useOwnerScopeStore((state) => state.loading);
    const user = useAuthStore((state) => state.user);
    const canEdit = scope === "self" || scope === "all" ? Boolean(user) : !membersLoading && members.find((item) => item.id === scope)?.accessLevel === "edit";
    ...
    }, [loadAssets, members, membersLoading, scope]);
    ...
    const openEdit = (asset: Asset) => {
        if (asset.accessLevel === "view") return message.warning("你只有查看权限");
    ...
    const readImageFile = async (file?: File) => {
        ...
        setImageDraft(draft);
        if (!form.getFieldValue("coverUrl")) form.setFieldValue("coverUrl", image.thumbnailUrl);
            ...
            <main className="min-h-0 flex-1 overflow-y-auto bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] px-6 py-8 [background-size:16px_16px] dark:bg-[radial-gradient(rgba(245,245,244,.14)_1px,transparent_1px)]">
                <div className="pb-8">
                    <div className="mx-auto max-w-5xl text-center">
                        <div className="flex flex-wrap items-center gap-3"><h1 className="text-4xl font-semibold tracking-tight text-stone-950 dark:text-stone-100">我的资产</h1><OwnerScopePicker /></div>
                ...
                <div className="mx-auto flex max-w-7xl flex-col gap-5">
                    {loadingError ? (
                        <Alert
                            ...
                            action={<Button size="small" onClick={() => void retryLoadAssets(scope)}>重试</Button>}
                        />
                    ) : null}
                    ...
                    <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                        {!loading || assets.length ? visibleAssets.map((asset) => (
                    ...
                    {!loading && !visibleAssets.length ? <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="没有找到资产" className="py-20" /> : null}
...
function AssetCard({ asset, onOpen, onEdit, onCopy, onDownload, onDelete }: { asset: Asset; onOpen: () => void; onEdit: () => void; onCopy: (asset: Asset) => void; onDownload: (asset: Asset) => void; onDelete: () => void }) {
    const cover = asset.coverUrl || (asset.kind === "image" ? asset.data.dataUrl : asset.kind === "video" ? asset.data.thumbnailUrl || "" : "");
                ...
                <div className="p-4">
                    <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                            <div className="flex items-center gap-2"><h2 className="line-clamp-1 text-sm font-semibold text-stone-950 dark:text-stone-100">{asset.title}</h2>{asset.ownerUsername ? <Tag className="m-0 shrink-0 text-[10px]">{asset.ownerUsername}</Tag> : null}</div>
...
function AssetDrawer({ asset, onClose, onCopy, onDownload }: { asset: Asset | null; onClose: () => void; onCopy: (asset: Asset) => void; onDownload: (asset: Asset) => void }) {
    const cover = asset ? asset.coverUrl || (asset.kind === "image" ? asset.data.dataUrl : asset.kind === "video" ? asset.data.thumbnailUrl || "" : "") : "";
    return (
