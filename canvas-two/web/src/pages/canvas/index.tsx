...
export default function CanvasPage() {
    ...
    const createAndEnter = async () => { try { enterProject(await createProject(`无限画布 ${projects.length + 1}`)); } catch (error) { message.error(error instanceof Error ? error.message : "创建画布失败"); } };
    ...
    useEffect(() => { autoOpenRef.current = false; void loadProjects(scope); }, [loadProjects, scope]);
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {selectedIds.length ? (
                            <>
                                ...
                                <Button disabled={!hydrated || !canEdit} onClick={() => setDeleteIds(selectedIds.filter((id) => projects.find((project) => project.id === id)?.accessLevel !== "view"))}>
                                    删除选中
                                </Button>
                            </>
                        ) : null}
                        {projects.length ? (
                            <Button disabled={!hydrated || !canEdit} onClick={() => setDeleteIds(projects.filter((project) => project.accessLevel !== "view").map((project) => project.id))}>
                                删除全部
                            </Button>
                        ) : null}
                        <Button disabled={!hydrated || !canEdit} icon={<FileUp className="size-4" />} onClick={() => inputRef.current?.click()}>
                            导入画布
                        </Button>
                        <Button disabled={!hydrated || !canEdit} type="primary" icon={<Plus className="size-4" />} onClick={() => void createAndEnter()}>
                            新建画布
                        </Button>
                    </div>
                </header>
