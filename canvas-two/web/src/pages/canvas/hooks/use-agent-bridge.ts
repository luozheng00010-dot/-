 ...
 * 供 API Agent 与本地 Codex 面板读取。除 applyAgentOps（配置节点插件宿主会用到）外均为内部实现。
 */
export function useAgentBridge(params: AgentBridgeParams) {
    const { projectId, readOnly = false, title, nodes, connections, selectedNodeIds, viewport, nodesRef, connectionsRef, selectedNodeIdsRef, viewportRef, generateNodeRef, setNodes, setConnections, setSelectedNodeIds, setSelectedConnectionId, setViewport, setContextMenu } =
        params;
    ...
    const applyAgentOps = useCallback(
        (ops?: CanvasAgentOp[]) => {
            ...
            if (readOnly && safeOps.length) throw new Error("当前画布为只读，不能执行写操作");
    ...
    useEffect(() => {
        setAgentCanvasContext({ snapshot: agentSnapshot, readOnly, applyOps: applyAgentOps, undoOps: undoAgentOps, canUndo: Boolean(agentUndoSnapshot) });
        return () => setAgentCanvasContext(null);
    }, [agentSnapshot, applyAgentOps, agentUndoSnapshot, readOnly, setAgentCanvasContext, undoAgentOps]);
