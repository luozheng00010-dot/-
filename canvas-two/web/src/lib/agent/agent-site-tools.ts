...
import { effectiveConfig, modelOptionLabel, modelOptionName, normalizeModelOptionValue, selectableModelsByCapability, useConfigStore } from "@/stores/use-config-store";
import { useWorkbenchAgentStore } from "@/stores/use-workbench-agent-store";
import { findImageGenerationTask, listImageGenerationTasks } from "@/services/image-generation-tasks";
...
type GenerationStatus = "idle" | "queued" | "running" | "succeeded" | "partial" | "failed";
type GenerationStatusItem = { id: string; taskId?: string; source: "canvas" | "image" | "video"; status: GenerationStatus; kind?: string; title?: string; prompt?: string; projectId?: string; createdAt?: string; updatedAt?: string; successCount?: number; failCount?: number; error?: string };
...
async function getGenerationStatus(input: SiteToolInput, canvasSnapshot?: CanvasAgentSnapshot | null) {
    ...
    if (includeWorkbench && scope !== "canvas" && scope !== "video") {
        const remoteTasks = taskId ? [await findImageGenerationTask(taskId)].filter(Boolean) : await listImageGenerationTasks("");
        remoteTasks.forEach((task) => tasks.push({ id: task.clientRequestId, taskId: task.id, source: "image", status: task.status === "cancelled" ? "failed" : task.status, kind: "image", title: task.prompt.slice(0, 30), prompt: compactPrompt(task.prompt), createdAt: task.createdAt, updatedAt: task.updatedAt, successCount: task.results.filter((item) => item.status === "succeeded").length, failCount: task.results.filter((item) => item.status === "failed").length, error: task.error }));
    }
    if (includeWorkbench && scope !== "canvas" && scope !== "image") {
        useWorkbenchAgentStore.getState().tasks.filter((task) => task.kind === "video").forEach((task) => {
    ...
    const summary: Record<GenerationStatus, number> = { idle: 0, queued: 0, running: 0, succeeded: 0, partial: 0, failed: 0 };
...
function generationStatusOrder(status: GenerationStatus) {
    return status === "running" ? 0 : status === "queued" ? 1 : status === "partial" ? 2 : 3;
}
...
function runImageWorkbench(input: SiteToolInput, navigate: NavigateFunction) {
    ...
    const channels = effectiveConfig().channels;
    const applied: Record<string, unknown> = {};
    if (typeof input.model === "string" && input.model.trim()) {
        const value = normalizeModelOptionValue(input.model, channels) || input.model;
...
async function addAsset(input: SiteToolInput) {
    ...
    if (kind === "image") {
        ...
        const id = store.addAsset({ kind: "image", title, coverUrl: stored.thumbnailUrl, tags, source, note, data: { dataUrl: stored.url, storageKey: stored.storageKey, width: stored.width, height: stored.height, bytes: stored.bytes, mimeType: stored.mimeType } });
