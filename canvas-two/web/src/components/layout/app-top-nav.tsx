import { Bot, LogOut, Menu, Shield, UserCircle } from "lucide-react";
...
export function AppTopNav() {
    ...
    const visibleNavigationTools = navigationTools;
                ...
                <header className="sticky top-0 z-20 h-14 shrink-0 border-b border-stone-200 bg-background/90 backdrop-blur-xl dark:border-stone-800">
                    <div className="mx-auto flex h-full max-w-7xl items-stretch justify-between gap-5 px-6">
                        ...
                        <div className="my-auto flex h-9 min-w-0 items-center justify-end gap-2 justify-self-end whitespace-nowrap">
                            ...
                            ], onClick: async ({ key }) => { if (key === "admin") navigate("/admin"); else { await logout(); navigate("/login", { replace: true }); } } }}>
                                <Button type="text" icon={<UserCircle className="size-4" />}>{user?.username}</Button>
                            </Dropdown>
                        </div>
                    </div>
                </header>
