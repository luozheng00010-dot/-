...
export function AppConfigPanel({ showDoneButton = false, initialTab = "channels" }: { showDoneButton?: boolean; initialTab?: ConfigTabKey }) {
    ...
    const finishConfig = () => {
        const ready = effectiveConfig.channels.some((channel) => channel.baseUrl.trim() && (channel.apiKey.trim() || channel.source === "remote") && channel.models.length);
    ...
    const updateChannels = (channels: ModelChannel[]) => saveConfig(withChannels(config, channels, serverChannels));
                ...
                items={[
                    {
                        ...
                        children: (
                            <div>
                                ...
                                <div className="mb-2 text-sm font-semibold">个人渠道</div>
                                ...
                                {serverChannels.length ? (
                                    <div className="mt-5">
                                        <div className="mb-2 text-sm font-semibold">管理员渠道</div>
                                        <div className="grid gap-2">
                                            {serverChannels.map((channel) => (
                                                <div key={channel.id} className="flex items-center justify-between gap-3 rounded-lg border border-stone-200 px-3 py-2.5 dark:border-stone-800">
                                                    <div className="min-w-0">
                                                        <div className="truncate text-sm font-medium">{channel.name}</div>
                                                        <div className="mt-0.5 text-xs text-stone-500">服务端代理 · {channel.models.length} 个模型</div>
                                                    </div>
                                                    <span className="shrink-0 text-xs text-stone-500">只读</span>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
...
function withChannels(config: AiConfig, channels: ModelChannel[], serverChannels: ModelChannel[]): AiConfig {
    const next: AiConfig = {
        ...config,
        channels,
    ...
    const selectableConfig = { ...next, channels: [...channels, ...serverChannels], models: modelOptionsFromChannels([...channels, ...serverChannels]) };
    return {
        ...next,
        imageModel: pickDefaultModel(selectableConfig, "image", config.imageModel),
        videoModel: pickDefaultModel(selectableConfig, "video", config.videoModel),
        textModel: pickDefaultModel(selectableConfig, "text", config.textModel),
        audioModel: pickDefaultModel(selectableConfig, "audio", config.audioModel),
    };
}
