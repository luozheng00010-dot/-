...
export function siteToolSummary(name: string, result: unknown) {
    ...
    if (name === "generation_get_status") {
        ...
        return `共 ${numberField(data, "total")} 个任务，排队 ${numberField(summary, "queued")}，运行中 ${numberField(summary, "running")}，成功 ${numberField(summary, "succeeded")}，部分完成 ${numberField(summary, "partial")}，失败 ${numberField(summary, "failed")}`;
    }
