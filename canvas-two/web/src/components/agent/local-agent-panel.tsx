...
export function LocalAgentPanel({ embedded, headless, autoConnect, modeSwitch }: { embedded?: boolean; headless?: boolean; autoConnect?: boolean; modeSwitch?: ReactNode }) {
    ...
    const handleToolCall = async (endpoint: string, token: string, payload: AgentPendingToolCall) => {
        if (isCanvasWriteTool(payload.name) && canvasContextRef.current?.readOnly) {
            const error = "当前画布为只读，不能执行写操作";
            addEventLog(`${toolName(payload.name)}失败`, error, payload);
            addMessage({ role: "tool", title: toolName(payload.name), text: error, detail: toolCallDetail(payload.name, payload.input, "failed", error) });
