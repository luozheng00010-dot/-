...
export function AgentChatTimeline({
    ...
    const messages = providedMessages || storedMessages;
...
export function AgentTaskProgress({ theme, busy, messages: providedMessages }: { theme: (typeof canvasThemes)[keyof typeof canvasThemes]; busy: boolean; messages?: AgentChatItem[] }) {
    const storedMessages = useAgentStore((state) => state.messages);
    const messages = providedMessages || storedMessages;
    const plan = busy ? currentPlanMessage(messages) : latestPlanMessage(messages);
    if (!plan) return null;
    return (
