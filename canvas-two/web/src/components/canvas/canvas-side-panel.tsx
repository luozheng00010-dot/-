...
import { App, Empty, Input, Pagination, Popconfirm, Select, Spin, Tag } from "antd";
import { useQuery } from "@tanstack/react-query";
import { BookOpen, Check, ChevronRight, Download, Eye, FileText, Image as ImageIcon, ListChecks, LoaderCircle, Music2, PackageOpen, Plus, RefreshCw, Search, Settings2, Square, Trash2, Type, Video } from "lucide-react";
...
import { imageThumbnailUrl, uploadImage } from "@/services/image-storage";
...
type PanelTab = "canvas" | "assets" | "prompts" | "products";
...
export function CanvasSidePanel({ nodes, selectedNodeIds, ownerId, readOnly, onFocusNode, onPreviewNode, onInsertAsset }: Props) {
    ...
    return (
        ...
        >
            <motion.aside
                ...
                data-canvas-no-zoom
            >
                <div className="hide-scrollbar flex shrink-0 items-center gap-5 overflow-x-auto whitespace-nowrap px-4 pt-3.5">
                    ...
                    <TabButton label="商品库" active={tab === "products"} theme={theme} onClick={() => setTab("products")} />
                </div>
                <div className="mt-2 min-h-0 flex-1 overflow-hidden">
                    {tab === "canvas" ? (
                        ...
                        <CanvasProductsTab ownerId={ownerId} readOnly={readOnly} onInsert={onInsertAsset} theme={theme} />
                    )}
                </div>
                ...
                {filtered.length ? (
                    <div className="space-y-1.5">
                        {filtered.map((node) => {
                            ...
                            const isVideo = node.type === CanvasNodeType.Video && node.metadata?.content;
                                ...
                                <div key={node.id} className={cn("group flex w-full items-center rounded-lg transition", active ? "" : "hover:bg-black/5 dark:hover:bg-white/5")} style={active ? { background: theme.toolbar.activeBg } : undefined}>
                                    <button type="button" onClick={() => (selectMode ? toggleChecked(node.id) : onFocusNode(node.id))} className="flex min-w-0 flex-1 items-center gap-3 px-2 py-2 text-left" title={selectMode ? undefined : "定位到节点"}>
                                        ...
                                        <span className="grid size-10 shrink-0 place-items-center overflow-hidden rounded-md">
                                            {isImage
                                                ? <img src={imageThumbnailUrl(node.metadata?.storageKey, node.metadata?.thumbnail || node.metadata?.content)} alt={node.title} loading="lazy" decoding="async" className="size-full object-cover" />
                                                : isVideo && node.metadata?.thumbnail
                                                    ? <img src={node.metadata.thumbnail} alt={node.title} loading="lazy" decoding="async" className="size-full object-cover" />
                                    ...
                                    {selectMode || (!isImage && !isVideo) ? null : (
...
function buildInsertPayload(asset: Asset): InsertAssetPayload {
    ...
    if (asset.kind === "video") return { kind: "video", url: asset.data.url, thumbnailUrl: asset.coverUrl || asset.data.thumbnailUrl, storageKey: asset.data.storageKey, title: asset.title, width: asset.data.width, height: asset.data.height };
        ...
        try {
            for (const file of files) {
                if (file.type.startsWith("image/")) {
                    ...
                    addAsset({ kind: "image", title: file.name || "图片", coverUrl: image.thumbnailUrl, tags: [], data: { dataUrl: image.url, storageKey: image.storageKey, width: image.width, height: image.height, bytes: image.bytes, mimeType: image.mimeType } });
                    added += 1;
                } else if (file.type.startsWith("video/")) {
                    const media = await uploadMediaFile(file, "video");
                    addAsset({ kind: "video", title: file.name || "视频", coverUrl: media.thumbnailUrl || "", tags: [], data: { url: media.url, thumbnailUrl: media.thumbnailUrl, storageKey: media.storageKey, width: media.width || 0, height: media.height || 0, bytes: media.bytes, mimeType: media.mimeType } });
                    added += 1;
                }
            }
...
function AssetCover({ asset }: { asset: Asset }) {
    ...
    if (asset.kind === "video") {
        const cover = asset.coverUrl || asset.data.thumbnailUrl;
        return cover ? <img src={cover} alt="" className="size-full object-cover transition duration-300 group-hover:scale-[1.04]" /> : <Video className="m-auto size-6 opacity-35" />;
    }
    return <img src={imageThumbnailUrl(asset.data.storageKey, asset.coverUrl || asset.data.dataUrl)} alt="" loading="lazy" decoding="async" className="size-full object-cover transition duration-300 group-hover:scale-[1.04]" />;
}
...
const CanvasProductsTab = memo(function CanvasProductsTab({ ownerId, readOnly, onInsert, theme }: { ownerId: string; readOnly: boolean; onInsert: (payload: InsertAssetPayload) => void; theme: CanvasTheme }) {
    const [keyword, setKeyword] = useState("");
    ...
    const query = useQuery({
        queryKey: ["canvas-products", ownerId, searchKeyword, page],
        queryFn: () => listProducts({ owner: ownerId, keyword: searchKeyword, page, pageSize: PRODUCT_PAGE_SIZE }),
        enabled: Boolean(ownerId),
        refetchOnMount: "always",
        refetchOnWindowFocus: true,
        refetchInterval: ({ state }) => state.data?.items.some((product) => product.status === "queued" || product.status === "running") ? 2_000 : false,
    });
    const products = query.data?.items || [];
    const total = query.data?.total || 0;

    const insertProduct = (product: Product) => {
        if (!product.resultImage || readOnly) return;
        onInsert({ kind: "image", dataUrl: product.resultImage.url, storageKey: product.resultImage.storageKey, productId: product.id, title: product.name });
    };

    return (
        <div className="flex h-full flex-col">
            <div className="px-3 pb-2.5 pt-1">
                <Input size="small" allowClear prefix={<Search className="size-3.5 text-stone-400" />} placeholder="搜索商品名称" value={keyword} onChange={(event) => setKeyword(event.target.value)} />
            </div>
            {query.isError ? (
                <div className="mx-3 mb-2 flex items-center gap-2 rounded-lg border border-red-500/25 px-3 py-2 text-xs text-red-500">
                    <span className="min-w-0 flex-1">商品加载失败</span>
                    <button type="button" className="flex shrink-0 items-center gap-1 transition hover:opacity-70" onClick={() => void query.refetch()}>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto px-2 pb-3">
                {query.isLoading && !products.length ? (
                    ...
                    <div className="space-y-1.5">
                        {products.map((product) => <ProductLibraryRow key={product.id} product={product} readOnly={readOnly} theme={theme} onInsert={() => insertProduct(product)} />)}
                    </div>
                ) : (
                    <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={searchKeyword ? "没有匹配的商品" : "暂无商品"} className="pt-16" />
                )}
            </div>
            {total > PRODUCT_PAGE_SIZE ? (
                <div className="shrink-0 border-t px-2 py-2.5" style={{ borderColor: theme.toolbar.border }}>
                    <Pagination size="small" simple current={page} pageSize={PRODUCT_PAGE_SIZE} total={total} showSizeChanger={false} onChange={setPage} />
                </div>
            ) : null}
        </div>
    );
...
function ProductLibraryRow({ product, readOnly, theme, onInsert }: { product: Product; readOnly: boolean; theme: CanvasTheme; onInsert: () => void }) {
    const active = product.status === "queued" || product.status === "running";
    const canInsert = Boolean(product.resultImage) && !readOnly;
    return (
        <div className="group relative flex items-center gap-2.5 rounded-lg px-2 py-2 transition hover:bg-black/5 dark:hover:bg-white/5">
            <span className="grid size-12 shrink-0 place-items-center overflow-hidden rounded-lg" style={{ background: theme.node.panel }}>
                {product.resultImage ? (
                    <img src={product.resultImage.thumbnailUrl || product.resultImage.url} alt={product.name} loading="lazy" decoding="async" className="size-full object-contain" />
                ) : active ? (
                    <LoaderCircle className="size-5 animate-spin opacity-55" />
                ) : (
                    <PackageOpen className="size-5 opacity-45" />
                )}
            </span>
            <span className="min-w-0 flex-1">
                <span className="block truncate text-sm font-medium leading-snug">{product.name}</span>
                {product.brand || product.productType ? <span className="mt-0.5 block truncate text-[11px] opacity-55">{[product.brand, product.productType].filter(Boolean).join(" · ")}</span> : null}
                <span className="mt-1 flex items-center gap-1.5">
                    <Tag color={PRODUCT_STATUS_COLOR[product.status]} className="m-0 !px-1.5 text-[10px] leading-4">{PRODUCT_STATUS_LABEL[product.status]}</Tag>
                    {product.resultImage && product.status === "failed" ? <span className="truncate text-[10px] opacity-45">可插入上次结果</span> : null}
                </span>
            </span>
            {product.resultImage ? (
                <button
                    ...
                    disabled={!canInsert}
                    onClick={onInsert}
                    className="grid size-7 shrink-0 place-items-center rounded-md opacity-0 transition hover:bg-black/10 group-hover:opacity-70 focus:opacity-70 disabled:cursor-not-allowed disabled:opacity-25 dark:hover:bg-white/10"
                    style={{ color: theme.toolbar.activeText }}
                    aria-label={readOnly ? "当前画布只读" : "插入商品主图"}
                    title={readOnly ? "当前画布只读" : "插入商品主图"}
                >
                    <Plus className="size-4" />
                </button>
            ) : null}
