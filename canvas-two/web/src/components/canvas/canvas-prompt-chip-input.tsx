...
function MentionMenu({ rect, references, activeIndex, theme, onSelect }: { rect: DOMRect | null; references: CanvasResourceReference[]; activeIndex: number; theme: (typeof canvasThemes)[keyof typeof canvasThemes]; onSelect: (reference: CanvasResourceReference) => void }) {
    ...
    const boundary = document.querySelector(".ant-modal-content")?.getBoundingClientRect();
    const minLeft = boundary ? boundary.left + 8 : 8;
    const maxRight = boundary ? boundary.right - 8 : window.innerWidth - 8;
    const minTop = boundary ? boundary.top + 8 : 8;
    const maxBottom = boundary ? boundary.bottom - 8 : window.innerHeight - 8;
    const left = clamp(anchor.left, minLeft, maxRight - menuWidth);
    const showAbove = anchor.bottom + gap + maxMenuHeight > maxBottom && anchor.top - gap - maxMenuHeight >= minTop;
    const top = showAbove ? Math.max(minTop, anchor.top - gap - maxMenuHeight) : Math.min(anchor.bottom + gap, maxBottom - maxMenuHeight);
