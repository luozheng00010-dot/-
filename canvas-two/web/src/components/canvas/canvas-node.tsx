...
type CanvasViewportFrame = { width: number; height: number; viewport: ViewportTransform };
...
type CanvasNodeProps = {
    ...
    readOnly?: boolean;
    getViewportFrame?: () => CanvasViewportFrame | null;
    ...
    onContentEditingChange?: (nodeId: string, editing: boolean) => void;
...
export const CanvasNode = React.memo(function CanvasNode({
    ...
    useEffect(() => {
        if (!isEditingContent) return;
        onContentEditingChange?.(data.id, true);
        return () => onContentEditingChange?.(data.id, false);
    }, [data.id, isEditingContent, onContentEditingChange]);
    ...
    const editingLayout = useMemo(
        () => isEditingContent && data.type === CanvasNodeType.Text ? textEditorLayout(data, scale, getViewportFrame?.()) : null,
        [data.height, data.position.x, data.position.y, data.type, data.width, getViewportFrame, isEditingContent, scale],
    );

    return (
        <div
            data-node-id={data.id}
            className={`node-element absolute flex select-none flex-col transition-shadow duration-200 ${isEditingContent ? "z-[120]" : isGroup ? "z-[5]" : isSelected ? "z-50" : "z-10"}`}
            style={{
                transform: `translate(${editingLayout?.x ?? data.position.x}px, ${editingLayout?.y ?? data.position.y}px)`,
                width: editingLayout?.width ?? data.width,
                height: editingLayout?.height ?? data.height,
        ...
        >
            {!isEditingContent && (isSelected || hovered || isEditingTitle) && (
            ...
            <div
                ...
                onMouseDown={readOnly || isEditingContent ? undefined : (event) => onMouseDown?.(event, data.id)}
                onDoubleClick={(event) => {
                    if (isBatchRoot) {
                        event.stopPropagation();
            ...
            >
                    ...
                    style={
                        {
                            ...
                            pointerEvents: readOnly || !contentInteractive ? "none" : undefined,
                ...
                {!readOnly && !isEditingContent ? <ResizeHandle corner="top-left" onMouseDown={handleResizeMouseDown} /> : null}
                {!readOnly && !isEditingContent ? <ResizeHandle corner="top-right" onMouseDown={handleResizeMouseDown} /> : null}
                {!readOnly && !isEditingContent ? <ResizeHandle corner="bottom-left" onMouseDown={handleResizeMouseDown} /> : null}
                {!readOnly && !isEditingContent ? <ResizeHandle corner="bottom-right" onMouseDown={handleResizeMouseDown} /> : null}
            </div>

            {!readOnly && !isGroup && !isEditingContent ? <ConnectionHandleDot side="left" visible={hovered || isSelected || isConnecting} onMouseDown={(event) => onConnectStart(event, data.id, "target")} /> : null}
            {!readOnly && !isGroup && !isEditingContent ? <ConnectionHandleDot side="right" visible={(definition?.hasSourceHandle ?? true) && data.type !== CanvasNodeType.Config && (hovered || isSelected || isConnecting)} onMouseDown={(event) => onConnectStart(event, data.id, "source")} /> : null}

            {!readOnly && !isEditingContent && showPanel && !isGroup && renderPanel ? <div className="absolute left-1/2 top-full z-[70] w-[600px] -translate-x-1/2 pt-4">{renderPanel(data)}</div> : null}
        </div>
    );
});

function textEditorLayout(node: CanvasNodeData, scale: number, frame?: CanvasViewportFrame | null) {
    const viewport = frame?.viewport;
    const safeScale = Math.max(viewport?.k || scale, 0.01);
    const targetWidth = Math.max(0, Math.min(720, (frame?.width || 784) - 64)) / safeScale;
    const targetHeight = Math.max(0, Math.min(520, (frame?.height || 600) - 80)) / safeScale;
    const width = Math.max(node.width, targetWidth);
    const height = Math.max(node.height, targetHeight);
    const preferredX = node.position.x + (node.width - width) / 2;
    const preferredY = node.position.y + (node.height - height) / 2;
    if (!frame || !viewport) return { x: preferredX, y: preferredY, width, height };

    const margin = 32 / safeScale;
    const left = -viewport.x / safeScale + margin;
    const top = -viewport.y / safeScale + margin;
    const right = (frame.width - viewport.x) / safeScale - margin - width;
    const bottom = (frame.height - viewport.y) / safeScale - margin - height;
    const fit = (value: number, min: number, max: number) => min <= max ? Math.min(Math.max(value, min), max) : (min + max) / 2;
    return { x: fit(preferredX, left, right), y: fit(preferredY, top, bottom), width, height };
}
...
function ErrorContent({ node, theme, onRetry }: Pick<NodeContentRendererProps, "node" | "theme" | "onRetry">) {
    const retryLabel = node.metadata?.workflow === "competitor-visual-replication" && node.metadata.workflowRole === "analysis" ? "重新分析" : "重试";
    return (
...
function VideoNodeContent({ node, theme }: NodeContentRendererProps) {
    ...
    const thumbnail = node.metadata.thumbnail || "";
    return thumbnail ? (
        <img src={thumbnail} alt={node.title || "视频"} className="block h-full w-full rounded-[18px] object-contain" draggable={false} data-canvas-no-zoom />
    ) : (
        <div className="flex h-full w-full flex-col items-center justify-center gap-3 rounded-[18px] bg-black/10" style={{ color: theme.node.placeholder }}>
            <Video className="size-7 opacity-35" />
    ...
    return (
        <BatchFrame batchCount={isBatchRoot ? batchCount : 0} batchExpanded={batchExpanded} batchOpening={batchOpening} batchRecovering={batchRecovering} onToggleBatch={onToggleBatch}>
            <div className="h-full w-full overflow-hidden rounded-3xl">
                <img
                    src={imageThumbnailUrl(node.metadata?.storageKey, node.metadata?.thumbnail || node.metadata?.content)}
                    alt={node.title}
                    loading="lazy"
                    decoding="async"
                    draggable={false}
