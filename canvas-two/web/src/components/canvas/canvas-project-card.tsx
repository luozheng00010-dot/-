...
export function CanvasProjectCard({ project }: { project: CanvasProject }) {
    ...
    const saveTitle = async () => {
        const saved = await renameProject(project.id, editingTitle);
        if (!saved) {
            message.error("画布当前由其他用户编辑，名称未保存");
            return;
        }
        stopEditing();
    };
        ...
        <article className="group flex min-h-44 cursor-pointer flex-col justify-between rounded-2xl bg-[#f1eee8] p-5 transition hover:bg-[#ebe6dc] dark:bg-white/5 dark:hover:bg-white/10" onClick={() => !editing && open()}>
            <div className="flex items-start gap-3">
                ...
                {editing ? (
                    ...
                    >
                        <div className="flex items-center gap-2"><h2 className="truncate text-xl font-semibold">{project.title}</h2>{project.ownerUsername ? <span className="shrink-0 rounded-full bg-white/70 px-2 py-0.5 text-[11px] text-stone-600 dark:bg-white/10 dark:text-stone-300">{project.ownerUsername}</span> : null}</div>
