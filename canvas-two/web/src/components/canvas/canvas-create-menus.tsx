...
export function ConnectionCreateOption({ theme, icon, title, description, badge, onClick }: { theme: (typeof canvasThemes)[keyof typeof canvasThemes]; icon: React.ReactNode; title: string; description?: string; badge?: string; onClick?: () => void }) {
    return (
        <button
            type="button"
        ...
        >
            ...
            <span className="min-w-0 flex-1">
                <span className="flex items-center gap-2 text-base font-semibold leading-5">
                    {title}
                    {badge ? (
                        <span className="rounded-full border px-1.5 py-0.5 text-[10px] font-medium leading-none" style={{ borderColor: theme.node.stroke, color: theme.node.muted }}>
                            {badge}
                        </span>
                    ) : null}
                </span>
