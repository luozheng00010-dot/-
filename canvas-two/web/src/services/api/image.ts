...
import { ensureUploadedImage, imageToDataUrl } from "@/services/image-storage";
import { submitImageGenerationTask, waitForImageGenerationTask, type ImageGenerationTask } from "@/services/image-generation-tasks";
...
type RequestOptions = { signal?: AbortSignal; clientRequestId?: string; ownerId?: string; context?: Record<string, unknown>; onTaskUpdate?: (task: ImageGenerationTask) => void };
...
function resolveImageResult(item: Record<string, unknown>) {
    const dataUrl = typeof item.b64_json === "string" && item.b64_json ? `data:image/png;base64,${item.b64_json}` : typeof item.url === "string" ? item.url : "";
...
function parseImagePayload(payload: ImageApiResponse) {
    ...
    const images =
        imageList
            ...
            .filter((value): value is NonNullable<ReturnType<typeof resolveImageResult>> => Boolean(value));
...
function storedImageHeaders(config: Pick<AiConfig, "source">) {
    return config.source === "remote" ? { "x-canvas-store-images": "1", "x-canvas-owner-id": selectedOwnerId() } : {};
}

function aiHeaders(config: AiConfig, contentType?: string, storeImages = false) {
    return {
        Authorization: `Bearer ${config.apiKey}`,
        ...(contentType ? { "Content-Type": contentType } : {}),
        ...(storeImages ? storedImageHeaders(config) : {}),
    };
}
...
function geminiHeaders(config: Pick<AiConfig, "apiKey" | "source">, storeImages = false) {
    return {
...
async function requestRemoteImageTask(config: AiConfig, prompt: string, references: ReferenceImage[], mask: ReferenceImage | undefined, options?: RequestOptions) {
    ...
    const uploadedReferences = await Promise.all(references.map(async (reference) => {
        const uploaded = await ensureUploadedImage(reference);
        return { mediaId: uploaded.storageKey.slice("media:".length), name: reference.name, type: uploaded.mimeType || reference.type };
    }));
    const uploadedMask = mask ? await ensureUploadedImage(mask) : undefined;
    const task = await submitImageGenerationTask({
        ownerId: options?.ownerId,
        clientRequestId: options?.clientRequestId,
        channelId: channel.id.replace(/^server:/, ""),
        operation: references.length ? "edit" : "generation",
        model: requestConfig.model,
        prompt,
        requestPrompt: withSystemPrompt(requestConfig, buildImageReferencePromptText(prompt, references)),
        count: Math.max(1, Math.min(15, Math.floor(Math.abs(Number(config.count)) || 1))),
        parameters: { ...(quality ? { quality } : {}), ...(size ? { size } : {}), ...(background ? { background } : {}) },
        references: uploadedReferences,
        ...(uploadedMask ? { maskMediaId: uploadedMask.storageKey.slice("media:".length) } : {}),
        context: options?.context,
    });
    options?.onTaskUpdate?.(task);
    const completed = await waitForImageGenerationTask(task.id, { signal: options?.signal, onUpdate: options?.onTaskUpdate });
    const images = completed.results.flatMap((result) => result.status === "succeeded" && result.image ? [result.image] : []);
    if (!images.length) throw new Error(completed.error || completed.results.find((result) => result.error)?.error || (completed.status === "cancelled" ? "任务已取消" : "生成失败，接口未返回原因"));
    return images;
}
...
async function requestGeminiImagesOnce(config: AiConfig, prompt: string, references: ReferenceImage[], options?: RequestOptions) {
    ...
    const response = await axios.post<GeminiPayload | ImageApiResponse>(
        ...
        { headers: geminiHeaders(config, true), signal: options?.signal },
    );
    return config.source === "remote" ? parseImagePayload(response.data as ImageApiResponse) : parseGeminiImagePayload(response.data as GeminiPayload);
}
...
export async function requestGeneration(config: AiConfig, prompt: string, options?: RequestOptions) {
    ...
    if (requestConfig.source === "remote") return requestRemoteImageTask(config, prompt, [], undefined, options);
    ...
    try {
        const response = await axios.post<ImageApiResponse>(
            ...
            {
                headers: aiHeaders(requestConfig, "application/json", true),
                signal: options?.signal,
            },
        );
...
export async function requestEdit(config: AiConfig, prompt: string, references: ReferenceImage[], mask?: ReferenceImage, options?: RequestOptions) {
    const requestConfig = resolveModelRequestConfig(config, config.model || config.imageModel);
    if (requestConfig.source === "remote") return requestRemoteImageTask(config, prompt, references, mask, options);
    const n = Math.max(1, Math.min(15, Math.floor(Math.abs(Number(config.count)) || 1)));
    ...
    try {
        const response = await axios.post<ImageApiResponse>(aiApiUrl(requestConfig, "/images/edits"), formData, { headers: aiHeaders(requestConfig, undefined, true), signal: options?.signal });
        const images = parseImagePayload(response.data);
        return images;
    } catch (error) {
        throw new Error(readAxiosError(error, "请求失败"));
