...
# 存储位置由管理页面配置，写入 config.toml 的 [storage] local_videos_dir。
# 读取时容忍历史遗留的非法值（回退默认目录），写入前严格校验。
MAX_STORAGE_DIR_LENGTH = 300


def configured_material_dir() -> str:
    """返回自定义素材存储目录；未配置或配置非法时返回空字符串。"""
    raw = str(config.storage.get("local_videos_dir", "") or "").strip()
    if not raw or len(raw) > MAX_STORAGE_DIR_LENGTH or "\x00" in raw:
        return ""
...
def uploaded_material_dir(create: bool = True) -> str:
    """新上传素材的落地目录：自定义位置优先，否则用默认 storage/local_videos。"""
    override = configured_material_dir()
    if override:
        if create and not os.path.exists(override):
            os.makedirs(override)
        return override
    return utils.storage_dir("local_videos", create=create)


def material_search_dirs() -> list:
    """按读取顺序返回素材检索目录：自定义位置在前，默认位置兜底。

    管理员更换存储位置后，历史素材仍留在原目录，必须继续可读，
    否则旧方案的预览、重匹配和渲染会集体找不到文件。
    """
    override = configured_material_dir()
    default_dir = utils.storage_dir("local_videos", create=False)
    dirs = []
    if override and override != default_dir:
        dirs.append(override)
    dirs.append(default_dir)
    return dirs


def find_material_file(file_key: str) -> str:
    """在检索目录中定位素材文件，返回完整路径；找不到返回空字符串。"""
    # 调用方均已用白名单正则约束 file_key，这里再取一次 basename，
    # 确保即使将来出现新的调用方也不会出现目录穿越。
    safe_key = os.path.basename(file_key or "")
...
def storage_settings() -> dict:
    default_dir = utils.storage_dir("local_videos", create=False)
    override = configured_material_dir()
    return {"dir": override or default_dir, "default_dir": default_dir, "custom": bool(override)}


def update_storage_dir(raw) -> dict:
    """更新素材存储位置并持久化到 config.toml；空值表示恢复默认。"""
    value = (raw or "").strip()
    if value:
        if len(value) > MAX_STORAGE_DIR_LENGTH or "\x00" in value:
            raise MaterialUploadError("存储路径过长或包含非法字符")
        normalized = os.path.normpath(value)
        if not os.path.isabs(normalized):
            raise MaterialUploadError("素材存储位置必须是绝对路径（例如 D:\\素材\\视频）")
        try:
            os.makedirs(normalized, exist_ok=True)
        except OSError as exc:
            raise MaterialServiceError("无法创建素材存储目录，请检查路径与权限") from exc
        if not os.access(normalized, os.W_OK):
            raise MaterialUploadError("素材存储目录不可写，请更换位置")
        config.storage["local_videos_dir"] = normalized
    else:
        config.storage.pop("local_videos_dir", None)
    config.save_config()
